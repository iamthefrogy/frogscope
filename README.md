...
all searchable and filterable from the Summary table's **Software** and
**Application** quick filters. Turns "what's our exposure to CVE-X" from a
Slack thread into a filtered grid.
...
Open **http://127.0.0.1:8099**, paste the key, and you're in. Light theme by
default - toggle to dark with the ☾ button, top right; it's remembered on
this browser after that.
...
these), then either press **Start scan** and wait, or press **Schedule scan**
to set up a recurring scan right from the same pane instead - see
[Scheduled scanning](#4-scheduled-scanning). A scan enumerates subdomains,
port-scans, probes what's live, and pulls DNS/network/TLS data automatically -
no extra toggles to remember; everything httpx can collect is collected, every
time.
...
|---|---|
| **Summary** | One big table of every asset, worst risk score first, every column shown by default. |
| **What to fix** | Every issue, ranked by severity, with evidence and how to fix it. Start here day-to-day. |
| **What changed** | New, gone, or modified since any previous scan of this project, grouped into Risk / Applications / Infrastructure. |
| **Configuration** | Run/manage scans and projects, set up scheduled scanning, see what a scan could and couldn't check. |

### 3. Summary - one leaderboard, six quick filters

**All assets** - every host, every column shown by default, fully filterable
and searchable, sorted by risk score first. Don't want a column? Uncheck it in
**Columns**, then **Remember columns** so that selection is what you see next
time you open the tab (per browser, not shared with anyone else). A row of
chips above the table narrows the same data to one slice, so you're never
looking at a different data source, just a filtered view of it:
...
- **Software** - a different shape of data (grouped by product, not one row
  per endpoint), so this chip opens its own page: web servers, detected
  technologies, end-of-life products, WordPress plugins.
...
Set one up from the scan pane directly (press **Schedule scan** instead of
**Start scan**), or manage existing schedules from **Configuration** - hourly,
daily, or weekly. Useful for anything meant to stay watched continuously
rather than scanned once and forgotten.

### 5. Exporting

**CSV / Excel** - from the Summary table, respects your current filter and
chip. There is no manual upload path - every scan goes through the UI (or
`frogscope scan` from the CLI), so a run's provenance is never in question.
